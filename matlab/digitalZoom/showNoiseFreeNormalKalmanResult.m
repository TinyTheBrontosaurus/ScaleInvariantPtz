function showDesiredResult
  ...
  figure('Name', 'Noise Free Normal Kalman Result' );
  hold off;
  ...
  xlabel( 'frame' );
  ylabel( 'position' );
  title( 'Noise Free Normal Kalman Result' );
  
  figure('Name', 'Noise Free Normal Kalman % error' );
  plot( abs(data.filtered-data.actual) ./ data.actual .*100 );
  xlabel( 'frame' );
  ylabel( '% error' );
  title( 'Noise Free Normal Kalman % error');
