function runSimulation
  ...
  while( run )
    ...
    %kf = scaleMeasurementNoise(kf,(3*myMeas^3));
    %kf = scaleProcessNoise(kf,(1)^1);
  ...
  figure('Name', 'Scaled Kalman Result' );
  hold off;
  ...
  xlabel( 'frame' );
  ylabel( 'position' );
  title( 'Scaled Kalman Result' );
  
  figure('Name', 'Scaled Kalman % error' );
  plot( abs(data.filtered-data.actual) ./ data.actual .*100 );
  xlabel( 'frame' );
  ylabel( '% error' );
  title( 'Scaled Kalman % error');
